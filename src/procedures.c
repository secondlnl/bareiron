...
PlayerData *getPlayerByName(int start_offset, int end_offset, uint8_t *buffer)
{
  for (int i = 0; i < MAX_PLAYERS; i++)
  {
    ...
    {
      if (player_data[i].name[j - start_offset] != buffer[j])
        break;
    }
...
uint8_t makeBlockChange(short x, uint8_t y, short z, uint8_t block)
{
  ...
  if (block == B_chest)
  {
    ...
    {
      if (i >= MAX_BLOCK_CHANGES)
        break; // No more space, trigger failBlockChange
...
void hurtEntity(int entity_id, int attacker_id, uint8_t damage_type, uint8_t damage)
{
  ...
  if (entity_id > 0)
  { // The attacked entity is a player
...
// Effectively ties player movement updates to the tickrate
  ...
  for (int i = 0; i < MAX_MOBS; i++)
  {
    ...
    if (new_x != old_x && (!isPassableBlock(getBlockAt(new_x, new_y + 1, old_z)) ||
    ...
    if (new_z != old_z && (!isPassableBlock(getBlockAt(old_x, new_y + 1, new_z)) ||
    ...
    if (new_x != old_x && new_z != old_z && (!isPassableBlock(block_above) || (!isPassableBlock(block) && !isPassableBlock(getBlockAt(new_x, new_y + 2, new_z)))))
    {
...
void broadcastChestUpdate(int origin_fd, uint8_t *storage_ptr, uint16_t item, uint8_t count, uint8_t slot)
{
  ...
  for (int i = 0; i < MAX_PLAYERS; i++)
  {
    ...
    if (memcmp(player_data[i].craft_items, &storage_ptr, sizeof(storage_ptr)) != 0)
      continue;
